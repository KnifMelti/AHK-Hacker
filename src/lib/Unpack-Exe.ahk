...
TryUnpackExe(exePath, logFile, writableDir := "") {
    ...
    ; Append to log
    FileAppend("`r`n===== UNPACKING ATTEMPT (UPX) =====`r`r`n", logFile, "UTF-8-RAW")
    FileAppend("Original file: " . exePath . "`r`r`n", logFile, "UTF-8-RAW")
    ...
    if (upxPath = "") {
        FileAppend("UPX not found - downloading...`r`n", logFile, "UTF-8-RAW")
        ShowProgress("Downloading UPX unpacker...", 1, "AHK-Hacker")
        upxPath := DownloadUpx(logFile)

        if (upxPath = "") {
            FileAppend("Failed to download UPX`r`n", logFile, "UTF-8-RAW")
            return ""
        }
    }

    FileAppend("Using UPX: " . upxPath . "`r`n", logFile, "UTF-8-RAW")
    ...
    FileAppend("Running UPX: " . cmd . "`r`n", logFile, "UTF-8-RAW")
    ...
    try {
        ...
        if (FileExist(unpackedPath)) {
            ...
            if (magic = "MZ") {
                FileAppend("UPX unpacking succeeded`r`n", logFile, "UTF-8-RAW")
                ShowProgress("Successfully unpacked with UPX", 0, "AHK-Hacker")
                return unpackedPath
            } else {
                try FileDelete(unpackedPath)
                FileAppend("UPX unpacking failed - invalid PE file`r`n", logFile, "UTF-8-RAW")
            }
        } else {
            FileAppend("UPX unpacking failed - no output file`r`n", logFile, "UTF-8-RAW")
        }
    } catch Error as e {
        try FileDelete(unpackedPath)
        FileAppend("UPX unpacking failed with error: " . e.Message . "`r`n", logFile, "UTF-8-RAW")
    }
...
DownloadUpx(logFile) {
    ...
    try {
        ...
        if (RegExMatch(jsonResponse, '"browser_download_url"\s*:\s*"([^"]*win64\.zip)"', &match)) {
            ...
            FileAppend("Failed to find win64.zip download URL from GitHub API`r`n", logFile, "UTF-8-RAW")
            return ""
        }
        ...
        if (RegExMatch(zipUrl, "upx-([0-9.]+)-win64", &verMatch)) {
            version := verMatch[1]
            FileAppend("Downloading UPX v" . version . "...`r`n", logFile, "UTF-8-RAW")
        } else {
            FileAppend("Downloading UPX...`r`n", logFile, "UTF-8-RAW")
        }
        ...
        if (!FileExist(tempZip)) {
            FileAppend("Failed to download UPX ZIP`r`n", logFile, "UTF-8-RAW")
            return ""
        }
        ...
        if (upxExePath = "") {
            ...
            FileAppend("UPX executable not found in ZIP`r`n", logFile, "UTF-8-RAW")
            return ""
        }
        ...
        FileAppend("UPX downloaded successfully to: " . finalPath . "`r`n", logFile, "UTF-8-RAW")
        return finalPath

    } catch Error as e {
        try FileDelete(binDir . "\upx_temp.zip")
        try DirDelete(binDir . "\upx_temp_extract", true)
        FileAppend("UPX download failed: " . e.Message . "`r`n", logFile, "UTF-8-RAW")
        return ""
    }
}

; Note: IsUPXPacked and PE parsing helpers (ReadDWord, ReadWord, ReadBytes, LogDetection)
; have been moved to lib/PE-Analysis.ahk to eliminate duplication
